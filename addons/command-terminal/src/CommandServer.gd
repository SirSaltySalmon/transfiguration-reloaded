...
func register_all_commands():
	register_command(
		CommandBuilder.new()
		.Literal("size").Literal("up").Callback(size_up)
		.Build()
	)
	
	register_command(
		CommandBuilder.new().Literal("size")
		.Literal("set").Validated("value", is_valid_int)
		.Tag_gn("int").Callback(set_size, ["value"])
		.Build()
	)
	
	register_command(
		CommandBuilder.new().Literal("money")
		.Branch()
		.Literal("up").Callback(money_up)
		.Build()
	)
	...
	register_command(
		CommandBuilder.new().Literal("money")
		.Literal("set").Validated("value", is_valid_int)
		.Tag_gn("int").Callback(set_money, ["value"])
		.Build()
	)

func register_all_parsers() -> void:
	register_parser("int", parse_int)
	return
